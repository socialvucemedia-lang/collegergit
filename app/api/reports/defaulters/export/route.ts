...
export async function GET(request: NextRequest) {
    try {
        ...
        // Generate CSV
        const headers = ['Roll Number', 'Name', 'Email', 'Semester', 'Section', 'Batch', 'Department', 'Total Classes', 'Attended', 'Percentage'];
        ...
        return new NextResponse(csv, {
            headers: {
                ...
                'Content-Disposition': `attachment; filename="defaulters_list_${new Date().toISOString().split('T')[0]}.csv"`
            }
        });
    } catch (error) {
